...
class AnimalPoseImage:
    def __init__(self, det_model_path: Optional[str] = None, pose_model_path: Optional[str] = None, torchscript_device=None):
        # Auto-detect GPU backend if not specified (CUDA/ROCm → MPS → CPU)
        if torchscript_device is None:
            if torch.cuda.is_available():
                torchscript_device = "cuda"
            elif hasattr(torch.backends, "mps") and torch.backends.mps.is_available():
                torchscript_device = "mps"
            else:
