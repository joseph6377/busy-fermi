"""
UmeAiRT Toolkit - Unit Tests for LTX Video Enhancer
------------------------------------------------------
...
class TestLTXVideoEnhancerDefinition(unittest.TestCase):
    """Tests for UmeAiRT_LTXVideoEnhancer node definition."""

    def setUp(self):
        from modules.ltx_enhancer import UmeAiRT_LTXVideoEnhancer
        self.cls = UmeAiRT_LTXVideoEnhancer
    ...
    def test_class_attributes(self):
        ...
        self.assertEqual(self.cls.FUNCTION, "enhance")
    ...
    def test_enhance_requires_frames(self):
        ...
        with self.assertRaises(ValueError):
            node.enhance(model_bundle=bundle, video_pipe=ctx)

    def test_enhance_requires_bundle_attrs(self):
...
class TestLTXVideoEnhancerTooltips(unittest.TestCase):
    """Every input should have a tooltip."""

    def test_all_required_inputs_have_tooltips(self):
        from modules.ltx_enhancer import UmeAiRT_LTXVideoEnhancer
        inputs = UmeAiRT_LTXVideoEnhancer.INPUT_TYPES()
        for key, spec in inputs["required"].items():
            if isinstance(spec, tuple) and len(spec) >= 2 and isinstance(spec[1], dict):
                self.assertIn("tooltip", spec[1],
                              f"Missing tooltip for '{key}' in LTXVideoEnhancer.required")
    ...
    def test_all_optional_inputs_have_tooltips(self):
        ...
        for key, spec in inputs["optional"].items():
            if isinstance(spec, tuple) and len(spec) >= 2 and isinstance(spec[1], dict):
                self.assertIn("tooltip", spec[1],
                              f"Missing tooltip for '{key}' in LTXVideoEnhancer.optional")
