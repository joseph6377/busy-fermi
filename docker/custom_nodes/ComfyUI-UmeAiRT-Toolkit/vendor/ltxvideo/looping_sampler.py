"""
Vendored from ComfyUI-LTXVideo/looping_sampler.py (Apache 2.0)

LTXVLoopingSampler — processes video in overlapping temporal/spatial tiles
for long-form or high-resolution video enhancement/upscaling.
"""
...
class LTXVLoopingSampler:
    """Process video in overlapping temporal + spatial tiles.

    Splits the input latent into spatial tiles, then processes each
    spatial tile's temporal chunks sequentially with overlap blending.
    Supports IC-LoRA guided sampling, keyframe conditioning, and
    AdaIN normalization for color consistency.
    """
    ...
    def _extract_latent_spatial_tile(self, latent_dict, v_start, v_end, h_start, h_end):
        ...
        if "noise_mask" in latent_dict and latent_dict["noise_mask"] is not None:
            tile_masks = latent_dict["noise_mask"][:, :, :, v_start:v_end, h_start:h_end]
            return {"samples": tile_samples, "noise_mask": tile_masks}
        else:
            return {"samples": tile_samples}

    def _extract_spatial_tile(
        self, latents, optional_guiding_latents, optional_negative_index_latents,
        optional_normalizing_latents, optional_keyframes,
        v_start, v_end, h_start, h_end, height_scale_factor, width_scale_factor,
    ):
        ...
        return (
            tile_latents, tile_guiding_latents, tile_negative_index_latents,
            tile_keyframes, tile_normalizing_latents,
        )

    def _process_temporal_chunks(
        self, tile_config: TileConfig, sampling_config: SamplingConfig,
        ...
        for i_temporal_tile, (start_index, end_index) in enumerate(
            zip(
                range(
                    ...
                    sampling_config.temporal_tile_size - sampling_config.temporal_overlap,
                ),
                range(
                    sampling_config.temporal_tile_size,
                    tile_config.tile_latents["samples"].shape[2]
            ...
            if tile_config.tile_guiding_latents is not None:
                guiding_latent_chunk = LTXVSelectLatents().select_latents(
                    ...
                    start_index,
                    min(end_index - 1, tile_config.tile_guiding_latents["samples"].shape[2] - 1),
                )[0]
            else:
            ...
            if start_index == 0:
                if tile_config.tile_guiding_latents is not None:
                    ...
                    tile_out_latents = LTXVBaseSampler().sample(
                        ...
                        ) * sampling_config.time_scale_factor + 1,
                ...
                tile_out_latents = LTXVExtendSampler().sample(
                    ...
                    num_new_frames=(
                        latent_chunk["samples"].shape[2] - sampling_config.temporal_overlap
                    ) * sampling_config.time_scale_factor,
                    frame_overlap=sampling_config.temporal_overlap * sampling_config.time_scale_factor,
                    guider=new_guider,
    ...
    def _create_spatial_weights(
        self, tile_shape, v, h, horizontal_tiles, vertical_tiles,
    ...
    def _calculate_tile_seed(self, first_seed, start_index, vertical_tiles,
                              horizontal_tiles, v, h, seed_offset):
    ...
    def _get_per_tile_value(self, value_list, tile_index):
        """Get a value from a per-tile configuration list."""
        return value_list[min(tile_index, len(value_list) - 1)]

    def _parse_comma_separated_string(self, config_string, default_value,
                                       converter_func, optional_total_size=None):
        """Parse a comma-separated configuration string into a list."""
        if config_string == "":
    ...
    def _prepare_guider_for_chunk(self, guider, optional_positive_conditionings, chunk_index):
        """Prepare the guider for a specific chunk."""
        if optional_positive_conditionings is not None:
            new_guider = copy.copy(guider)
            positive, negative = guider.raw_conds
            conditioning_index = min(chunk_index, len(optional_positive_conditionings) - 1)
    ...
    def _calculate_keyframe_per_tile_indices(
        ...
        Returns a list of tuples (temporal_tile_index, in_tile_index).
        """
        result = []
        for keyframe_index in keyframe_indices:
            ...
            while True:
                ...
                if keyframe_index <= tile_end:
                    in_tile_index = keyframe_index - tile_start - 7
                    ...
                    if in_tile_index < temporal_overlap:
                        ...
                        if tile_index == 0:
                            ...
                            in_tile_index = keyframe_index - prev_start - 7
    ...
    def sample(
        ...
        """Sample video latents using overlapping temporal + spatial tiling.
        ...
        Args:
            ...
            guider: CFG/STG guider.
            latents: Input latents (can be guiding latents for enhancement).
            guiding_strength: Strength of guidance on latents.
            adain_factor: AdaIN normalization factor.
            temporal_tile_size: Size of temporal tiles in pixel frames.
            temporal_overlap: Overlap between temporal tiles.
            temporal_overlap_cond_strength: Conditioning strength for overlap.
            horizontal_tiles: Number of horizontal spatial tiles.
            vertical_tiles: Number of vertical spatial tiles.
            spatial_overlap: Overlap between spatial tiles.

        Returns:
            Tuple of (output_latent_dict,).
        """
        ...
        if (
            ...
            raise ValueError(
                "LoopingSampler does not support Audio Visual latents. "
                "Please only use video latents."
            )
        ...
        keyframe_indices = self._parse_comma_separated_string(
            optional_cond_image_indices, "0", int,
        ...
        if optional_guiding_latents is not None:
            ...
            assert samples.shape[2] == guide.shape[2], \
                "Frames in latents and optional_guiding_latents must match"
            assert (
                samples.shape[3] % guide.shape[3] == 0
                and samples.shape[4] % guide.shape[4] == 0
            ), "Height/width ratio of latents and guiding_latents must be integer"
        ...
        for v in range(vertical_tiles):
            for h in range(horizontal_tiles):
                ...
                (
                    ...
                    latents, optional_guiding_latents,
                    optional_negative_index_latents, optional_normalizing_latents,
                    optional_keyframes,
                ...
                sampling_config = SamplingConfig(
                    ...
                    optional_negative_index=(
                        ...
                        if tile_negative_index_latents is not None else -1
                    ),
                ...
                tile_out_latents = self._process_temporal_chunks(
                    tile_config, sampling_config, model_config,
                )

                if final_output is None:
                    out_temporal = tile_out_latents["samples"].shape[2]
                    final_output = torch.zeros(
                        batch, channels, out_temporal, height, width,
                ...
                tile_weights = self._create_spatial_weights(
                    tile_samples.shape,
                    v, h, horizontal_tiles, vertical_tiles,
                    spatial_overlap, final_output.device, final_output.dtype,
                )
