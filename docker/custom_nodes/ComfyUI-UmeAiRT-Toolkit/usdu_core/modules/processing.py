...
def sample(model, seed, steps, cfg, sampler_name, scheduler, positive, negative, latent, denoise, custom_sampler, custom_sigmas, suppress_preview=False):
    ...
    # Silent Sampling (Progress events only — no preview images in the node UI)
    if suppress_preview:
        # Replicate common_ksampler logic but with a progress-only callback (no preview)
        ...
        # Progress-only callback: sends WebSocket progress events without preview images
        ...
        samples_tensor = comfy.sample.sample(
            ...
            force_full_denoise=False, noise_mask=noise_mask, callback=progress_callback, disable_pbar=disable_pbar, seed=seed
        )
